...
import { getUsageTrackings } from 'apiSdk/usage-trackings';
...
function DashboardPage() {
  ...
  const userProfilesStat = useSWR<PaginatedInterface<UserProfilesInterface>>(
    () => `/user-profiles`,
    () => getUserProfiles({ limit: 1, offset: 0 }),
  );

  const usageTrackingStat = useSWR<PaginatedInterface<UsageTrackingInterface>>(
    () => `/usage-trackings`,
    () => getUsageTrackings({ limit: 1, offset: 0 }),
  );

  const reservationsStat = useSWR<PaginatedInterface<ReservationsInterface>>(
    () => `/reservations`,
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'User Profiles',
      result: userProfilesStat?.data?.totalCount,
      isLoading: userProfilesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Usage Trackings',
      result: usageTrackingStat?.data?.totalCount,
      isLoading: usageTrackingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reservations',
      result: reservationsStat?.data?.totalCount,
      isLoading: reservationsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
